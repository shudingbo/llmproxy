...
- **会话探测（会话消息监控）**：Sessions 页操作栏新增「探测」按钮，点击弹出 `el-drawer` 实时展示该会话与 LLM 交互的全部消息（历史 + 实时 + 流式增量）。**被动监控**：不发起任何探测请求，仅旁路记录并推送既有业务流量（`/v1/chat/completions`、`/v1/responses`（convert / native 两模式）、`/api/chat`）
  ...
  - **思考内容（think / reasoning）**：推理模型（DeepSeek 等）的思考过程（`reasoning_content`）与正文同口径旁路记录、独立通道展示——
    - **存储**：`session_messages.reasoning` 列（`TEXT NOT NULL DEFAULT ''`；旧表打开时自动 `ALTER TABLE ADD COLUMN` 迁移，既有行取空串）。reasoning **不参与** `(session_key, content_hash)` 去重哈希（同一逻辑消息有的请求回显思考、有的不回显，不产生重复行）；请求侧去重命中且旧行 reasoning 为空、新值非空时**回填**旧行（客户端多轮对话后一次才回显 `reasoning_content` 的场景）
    - **流式**：`AssistantStreamRecorder` 双通道——chat 口径 `delta.reasoning_content` 走 `think` 通道（与 `content` 同一 choice、思考先于正文到达）；responses 口径 `response.reasoning_text.delta` / `response.reasoning_summary_text.delta`（原生透传流；对应 `.done` 全文兜底，语义同 `output_text.done`）。事件契约扩展：`assistant_delta` 增加 `channel: 'think' | 'content'`；`assistant_done` 增加 `reasoning`（完整思考文本，中途订阅者可凭此补块）；`message` 事件（历史回放 / 新写入 / 非流式）增加 `reasoning`（无则空串）。仅思考无正文的回答（如思考中被中断）同样落库留痕
    - **tap 覆盖**：chat 流式 / 非流式（`message.reasoning_content`）、Ollama 流式（上游 chat SSE 同口径）/ 非流式（取自原始 OpenAI 响应，Ollama 形状转换后不含该字段）、Responses 原生（流式 reasoning_* 事件 / 非流式 `output` 中 `type=reasoning` 项的 summary 文本归并到紧随的 message 项）、Responses convert 非流式（tap 改取原始 chat 响应保留思考）。**已知边界**：Responses convert 流式路径的思考不记录——`createResponsesStream` 转换器以简单可靠为准不携带 `reasoning_content`
    - **前端**：抽屉内 assistant 块中以独立「思考」子区块展示（`el-tag` 标记 + markdown-it 渲染 + 浅底弱化色调）；流式期间 think delta 逐字增长（与正文同机制，v-html 原位重渲染）；`assistant_done` 的完整思考文本为权威值覆盖中途订阅者漏收的开头片段
    - **测试**：`test/monitor/` 新增 15 例（双通道解析 / done 兜底 / 迁移补列 / 去重回填 / 门面事件 / responses reasoning 项归并）+ `test/server/session-monitor.test.ts` 新增 1 例端到端（思考 delta 先于正文实时到达 + `assistant_done.reasoning` + 历史回放携带 reasoning）
