...
- **API Key 鉴权（Bearer Token）**：
  - **配置开关**：`llmproxy.jsonc` 新增 `auth` 节（`{ enabled?: boolean, keyBytes?: number, cleanupRetentionDays?: number }`，缺省 `disabled`）。全局开关开启时，所有 `/v1/*`（含 `/v1/models`/`/v1/chat/completions`/`/v1/responses`/`/v1/embeddings`/`/v1/rerank`）与 `/api/*`（含 `/api/version`/`/api/tags`/`/api/chat`/`/api/show`）请求必须携带 `Authorization: Bearer <key>`，否则返回 401。管理端 `/admin/api/*` 不挂鉴权（由部署层防护）。开关读取每次请求走 `store.get()`，支持热更新；关闭时中间件完全旁路、零 IO
