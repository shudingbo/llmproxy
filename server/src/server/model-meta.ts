// 下游模型列表的上下文聚合：为每个下游别名（聚合分组）计算候选上游 max_context_length 的最小值
...
/**
 * 解析单个别名的聚合上下文：
 * 遍历候选上游，收集其 max_context_length（仅统计有限正整数）；
...
export function resolveAliasNctx(config: Config, alias: string): { n_ctx: number } | undefined {
  ...
  for (const candidate of config.downstreamModels[alias]) {
    const value = config.upstreams.find((u) => u.id === candidate.upstreamId)?.max_context_length
